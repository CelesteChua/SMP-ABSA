...
def print_split_status(outer_loop: int,split_stats: dict,val_score: float, test_score: float):
    '''
...
def learning_to_split(data: Dataset,
    ...
    for element in return_order:
        ...
        elif element == 'predictor':

            # return the learned predictor
            predictor.load_state_dict(best_split['predictor'])
            outputs.append(predictor)
